...
| 私信       | 1. **设计数据模型**：设计用于存储聊天消息的数据模型。使用 Firebase 实时数据库来存储这些消息。在数据模型中，定义每个消息的属性，如消息 ID、发送者 ID、接收者 ID、消息时间戳和消息文本。<br /> 2. **实现消息发送**：使用实时数据库 API 来发送和接收消息。在发送消息时，将消息数据保存到 Firebase 中，并在 Firebase 数据库中设置适当的监听器以接收新消息的通知。                                                    |
