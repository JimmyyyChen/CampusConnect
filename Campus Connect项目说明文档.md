...
| 私信       | 1. **设计数据模型**：设计用于存储聊天消息的数据模型。使用 Firebase 实时数据库来存储这些消息。在数据模型中，定义每个消息的属性，如消息 ID、发送者 ID、接收者 ID、消息时间戳和消息文本。<br /> 2. **实现消息发送**：使用实时数据库 API 来发送和接收消息。在发送消息时，将消息数据保存到 Firebase 中，并在 Firebase 数据库中设置适当的监听器以接收新消息的通知。  <br/>3. 在`Chat_Screen`的`ListView.builder`中，为每条消息记录都生成了一个ChatBubble组件，用于展示消息内容。当消息发送方为当前用户时，ChatBubble显示在右侧，否则显示在左侧。聊天记录采用了倒序排列，最新的消息显示在最上面。                                                  |
...
| 功能点                                                   | 设计与实现 |
| -------------------------------------------------------- | ---------- |
| 收到私信、信息被点赞、回复、关注用户发布新信息时接收通知 | 1. fcmToken是在注册时记录下来的,一个设备有唯一的fcmToken <br> 2. 接收到信息后, ChatProvider.dart会对收到的`RemoteMessage message`进行处理,然后使用`flutterLocalNotificationsPlugin`库进行消息的显示    |
| 个人中心界面进入通知消息列表，查看通知详情、回复私信     | 1.使用Firestore实时查询当前用户的消息记录，使用`StreamBuilder`来监听查询结果的变化。<br>2. 对于私信通知，解析消息记录中的UID，跳转到私信页面。       |

### 搜索

| 功能点           | 设计与实现 |
| ---------------- | ---------- |
| 搜索控件         |  1. 对每个关键词，分别在“users”集合和“posts”集合中进行搜索。<br> 4. 将结果集中的重复项去除，然后将结果分别展示在“Users”和“Posts”两个子列表中。       |
| 多关键词联合搜索 |  1. 在搜索框中输入关键词后，需要将关键词按空格分割为多个单独的关键词，并对每个关键词分别进行搜索。  |
